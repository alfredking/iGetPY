...
weight_edges = [(edge[0], edge[1], 1) for edge in edges]
G.add_weighted_edges_from(weight_edges)

# pagerank_list = nx.pagerank(G, alpha=1)
pagerank = nx.pagerank(G)  # alpha默认是0.85,代表用户有 15% 的概率随机跳转,85%的可能性是通过链接浏览网页
...
for index, row in file.iterrows():
    aliases[row['Alias']] = row['PersonId']  # 通过这一步操作可以通过别名找到PersonId
# 读取人名文件
file = pd.read_csv("./input/Persons.csv")
persons = {}
for index, row in file.iterrows():
    persons[row['Id']] = row['Name']  # 通过这一步操作可以通过PersonId找到真名
